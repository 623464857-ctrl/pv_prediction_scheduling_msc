...
实验名称: 基础预测模型对比 — LSTM
实验目的: 建立单向时序深度学习基线
运行方式: python experiments/prediction/step2_baseline_models/run_exp_p02_train_lstm.py
"""
...
from exp_p02_torch_utils import LSTMRegressor, get_device, make_loader, predict, train_with_early_stop  # noqa: E402

LOG_NAME = "EXP-P02_LSTM.log"


def main() -> None:
    logger = setup_logger("EXP-P02-LSTM", LOG_NAME)
    cfg = load_config()
    ...
    model, history = train_with_early_stop(
        LSTMRegressor(n_features, hidden=64, num_layers=2, dropout=0.2),
        train_loader,
        val_loader,
        lr=1e-3,
        max_epochs=50,
