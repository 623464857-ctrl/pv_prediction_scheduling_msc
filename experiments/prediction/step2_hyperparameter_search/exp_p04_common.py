...
LOG_DIR = PROJECT_ROOT / "logs" / "prediction" / "step2_hyperparameter_search"
...
def _sample_dir(horizon: int, lookback: int | None = None, dataset: str = "default") -> Path:
    """解析样本目录: h{horizon}_lb{lookback}，兼容旧版目录命名。
    
    Args:
        horizon: 预测步数
        lookback: 回看步数
        dataset: 数据集名称 ("default" 或 "mingyuehu")
    """
    if lookback is None:
        lookback = load_config("exp_p04_base.json")["lookback"]

    if dataset == "mingyuehu":
        new_dir = SAMPLES_DIR / f"mingyuehu_h{horizon}_lb{lookback}"
    else:
...
def load_sample_dir(horizon: int, lookback: int | None = None) -> Path:
    """返回样本目录（默认数据集）。"""
    return _sample_dir(horizon, lookback, dataset="default")


def load_mingyuehu_sample_dir(horizon: int, lookback: int | None = None) -> Path:
    """返回明月湖样本目录。"""
    if lookback is None:
        lookback_map = {1: 16, 4: 48, 16: 96}
        lookback = lookback_map.get(horizon, 16)
    return _sample_dir(horizon, lookback, dataset="mingyuehu")
