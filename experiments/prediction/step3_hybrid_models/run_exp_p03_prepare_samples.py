"""
实验编号: EXP-P03-prepare
实验名称: 混合深度学习模型对比 — 样本构造与数据划分
实验目的: 从 Site_1 清洗长表构造滑动窗口样本，按时序 56/14/30 划分，并用训练集拟合标准化器
所属方向: prediction / step3_hybrid_models
输入路径: data/prediction/step1_preprocessing/processed/stations/Site_1_preprocessed.csv
输出路径: data/prediction/step3_hybrid_models/samples/
运行方式: python experiments/prediction/step3_hybrid_models/run_exp_p03_prepare_samples.py
...
def main() -> None:
    logger = setup_logger("EXP-P03-prepare", LOG_NAME)
    cfg = load_config()
    ...
    n = len(y)
    n_total_train_val = int(n * (cfg["train_ratio"] + cfg["val_ratio"]))
    n_train = int(n_total_train_val * cfg["train_ratio"] / (cfg["train_ratio"] + cfg["val_ratio"]))
    n_val = n_total_train_val - n_train
    n_test = n - n_total_train_val
    ...
    with open(SAMPLES_DIR / "window_meta.json", "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2, ensure_ascii=False)

    logger.info("样本总数=%d | 训练=%d 验证=%d 测试=%d", n, n_train, n_val, n_test)
    logger.info("序列输入=[%d, %d]", lookback, len(feature_cols))
    ...
    append_log_summary(
        ...
        [
            ...
            f"- 特征数: {len(feature_cols)}, 序列输入=[{lookback}, {len(feature_cols)}]",
            f"- 划分: train={n_train}, val={n_val}, test={n_test} (时序顺序, 无 shuffle)",
            "- 标准化: StandardScaler, 仅训练集拟合",
            "- 产出: samples/*.npy, scaler_params.json, window_meta.json",
            "=" * 60,
        ],
    )
    logger.info("EXP-P03-prepare 结束")
