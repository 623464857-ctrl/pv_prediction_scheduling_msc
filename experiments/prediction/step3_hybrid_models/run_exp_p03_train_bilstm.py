"""
实验编号: EXP-P03-BiLSTM
实验名称: 混合深度学习模型对比 — BiLSTM 基准
实验目的: 作为双向时序基准，与 CNN-BiLSTM 对比验证 CNN 局部特征提取的增益
运行方式: python experiments/prediction/step3_hybrid_models/run_exp_p03_train_bilstm.py
"""
...
from exp_p03_models import BiLSTMRegressor
from exp_p03_torch_utils import get_device, make_loader, predict, train_with_early_stop

LOG_NAME = "EXP-P03_BiLSTM.log"


def main() -> None:
    logger = setup_logger("EXP-P03-BiLSTM", LOG_NAME)
    cfg = load_config()
    ...
    model, history = train_with_early_stop(
        BiLSTMRegressor(n_features=n_features, hidden=64, layers=2, dropout=0.2),
        train_loader,
        val_loader,
        lr=1e-3,
        max_epochs=50,
    ...
    plot_loss_curve(METRICS_DIR / "bilstm_train_history.csv", "BiLSTM Training Loss", FIGURES_DIR / "loss_bilstm.png")
    ts = load_test_timestamps()
    df_pred = pd.DataFrame({"timestamp": ts, "y_true": data["y_test"], "y_pred": y_pred})
    plot_pred_curve(df_pred, "BiLSTM — test predictions", FIGURES_DIR / "pred_bilstm.png", n_points=5 * 96)
    ...
    append_log_summary(
        ...
        [
            ...
            "- 图表: figures/loss_bilstm.png, figures/pred_bilstm.png",
            "=" * 60,
        ],
    )
    logger.info("EXP-P03-BiLSTM 结束")
