...
用法：
    python -m experiments.prediction.step4_optuna_hybrid.plot_site4_analysis
"""
...
def load_site4_data():
    """加载Site1预处理后的完整数据"""
    csv_path = DATA_DIR / "Site_4_with_wrf.csv"
...
def plot_site4_data_overview(df: pd.DataFrame, output_dir: Path):
    """绘制Site1数据概览图"""
    ...
    axes[0].set_title("Site 4 功率 (p.u.) - 2019年", fontsize=14)
    ...
    axes[1].set_title("Site 4 功率 (p.u.) - 2020年", fontsize=14)
    ...
    axes[2].set_title("Site 4 功率 (p.u.) - 完整时间序列 (2019-2020)", fontsize=14)
    axes[2].set_xlabel("时间")
    axes[2].set_ylabel("功率 (p.u.)")
    axes[2].grid(True, alpha=0.3)

    plt.tight_layout()
    fig.savefig(output_dir / "site4_power_overview.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"已保存: {output_dir / 'site4_power_overview.png'}")


def plot_site4_monthly_pattern(df: pd.DataFrame, output_dir: Path):
    """绘制月度功率模式图"""
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    plt.suptitle("Site 4 各月平均日功率曲线", fontsize=16, y=1.02)
    plt.tight_layout()
    fig.savefig(output_dir / "site4_monthly_pattern.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"已保存: {output_dir / 'site4_monthly_pattern.png'}")


def plot_site4_data_quality(df: pd.DataFrame, output_dir: Path):
    """绘制数据质量分析图"""
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    plt.tight_layout()
    fig.savefig(output_dir / "site4_data_quality.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"已保存: {output_dir / 'site4_data_quality.png'}")


def plot_site4_feature_correlation(df: pd.DataFrame, output_dir: Path):
    """绘制特征相关性热力图"""
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    if len(key_features) > 2:
        ...
        ax.set_title("Site 4 关键特征相关性热力图", fontsize=14)

        plt.tight_layout()
        fig.savefig(output_dir / "site4_feature_correlation.png", dpi=150, bbox_inches="tight")
        plt.close()
        print(f"已保存: {output_dir / 'site4_feature_correlation.png'}")


def plot_site4_wrf_features(df: pd.DataFrame, output_dir: Path):
    """绘制WRF气象特征对比图"""
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    plt.suptitle("Site 4 WRF气象特征 (2020年6-8月样本)", fontsize=14, y=1.02)
    plt.tight_layout()
    fig.savefig(output_dir / "site4_wrf_features.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"已保存: {output_dir / 'site4_wrf_features.png'}")
...
def plot_horizon_prediction_curves(horizons: list, output_dir: Path):
    ...
    plt.suptitle("Site 4 CNN-BiLSTM 预测曲线对比\n验证集时间范围: 2020年10月5日-12日",
                 fontsize=16, fontweight="bold", y=0.995)
    plt.tight_layout(rect=[0, 0, 1, 0.98])

    output_path = output_dir / "site4_h1_h4_h16_prediction_curves.png"
...
def plot_horizon_monthly_comparison(horizons: list, output_dir: Path):
    ...
    for month_key, month_label in months:
        ...
        plt.suptitle(f"Site 4 CNN-BiLSTM 预测曲线 - {month_label} 2020年\n"
                    f"时间范围: {start_date[:10]} 至 {end_date[:10]}",
                    fontsize=16, fontweight="bold", y=0.995)
        plt.tight_layout(rect=[0, 0, 1, 0.98])

        output_path = output_dir / f"site4_predictions_{month_key}_month.png"
        fig.savefig(output_path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"已保存: {output_path}")
...
def plot_all_horizons_single_figure(horizons: list, output_dir: Path):
    ...
    plt.suptitle("Site 4 CNN-BiLSTM 预测曲线对比 (H1/H4/H16)\n"
                "真实值(黑色实线) vs 预测值(彩色虚线)",
                fontsize=16, fontweight="bold", y=0.98)

    output_path = output_dir / "site4_h1_h4_h16_all_months.png"
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"已保存: {output_path}")
...
def plot_quarterly_prediction_curves(horizons: list, output_dir: Path):
    ...
    for quarter in quarters:
        ...
        plt.suptitle(f"Site 4 CNN-BiLSTM 预测曲线 - {quarter['title']}\n"
                    f"真实值(黑色实线) vs 预测值(彩色虚线)",
                    fontsize=16, fontweight="bold", y=0.98)

        output_path = output_dir / f"site4_quarterly_predictions_{quarter['name'].lower()}.png"
        fig.savefig(output_path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"已保存: {output_path}")
...
def main():
    ...
    figures_output = FIGURES_DIR / "site4_analysis"
    ...
    if generate_analysis:
        ...
        plot_site4_data_overview(df, figures_output)
        plot_site4_monthly_pattern(df, figures_output)
        plot_site4_data_quality(df, figures_output)
        plot_site4_feature_correlation(df, figures_output)
        plot_site4_wrf_features(df, figures_output)
